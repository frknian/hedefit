create table if not exists public.workout_program_collections (
  ...
  source text not null check (source in ('regional', 'custom', 'assessment')),
...
create index if not exists workout_program_collections_user_updated_idx
  on public.workout_program_collections(user_id, updated_at desc);
...
select user_id, 'Test Sonucu Programım', 'assessment', coalesce(workouts->0->>'area', ''), workouts, true
from public.workout_plans
where jsonb_typeof(workouts) = 'array' and jsonb_array_length(workouts) > 0
  and not exists (select 1 from public.workout_program_collections c where c.user_id = workout_plans.user_id);
