-- Android istemcisi için favori öğünler ve zengin set türleri.
...
create policy "Users manage own favorite meals" on public.favorite_meals
...
create policy "Users manage own routes" on public.route_activities
  for all using (auth.uid() = user_id and public.account_is_active())
  with check (auth.uid() = user_id and public.account_is_active());
